...
/**
 * Definition of the grade_gradeconfigwizard_report class
 *
...
function grade_report_gradeconfigwizard_settings_definition(&$mform) {
    global $CFG;
    ...
    if (empty($CFG->grade_report_gradeconfigwizard_showrank)) {
        $options[-1] = get_string('defaultprev', 'grades', $options[0]);
    } else {
        $options[-1] = get_string('defaultprev', 'grades', $options[1]);
    }

    $mform->addElement('select', 'report_gradeconfigwizard_showrank', get_string('showrank', 'grades'), $options);
    $mform->addHelpButton('report_gradeconfigwizard_showrank', 'showrank', 'grades');
    ...
    if (!array_key_exists($CFG->grade_report_gradeconfigwizard_showtotalsifcontainhidden, $options)) {
        $options[-1] = get_string('defaultprev', 'grades', $options[0]);
    } else {
        $options[-1] = get_string('defaultprev', 'grades', $options[$CFG->grade_report_gradeconfigwizard_showtotalsifcontainhidden]);
    }

    $mform->addElement('select', 'report_gradeconfigwizard_showtotalsifcontainhidden', get_string('hidetotalifhiddenitems', 'grades'), $options);
    $mform->addHelpButton('report_gradeconfigwizard_showtotalsifcontainhidden', 'hidetotalifhiddenitems', 'grades');
}
...
function gradereport_gradeconfigwizard_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    $coursecontext = context_course::instance($course->id);
    if (grade_report_gradeconfigwizard::check_access($systemcontext, $coursecontext, $usercontext, $course, $user->id)) {
        $url = new moodle_url('/grade/report/gradeconfigwizard/index.php', array('userid' => $user->id, 'id' => $course->id));
        $node = new core_user\output\myprofile\node('reports', 'grades', get_string('gradesgradeconfigwizard', 'gradereport_gradeconfigwizard'),
                null, $url);
        $tree->add_node($node);
    }
}

function search_selfitem($availablegradeitems, $gradeitemidtarget): int {
    $size = sizeof($availablegradeitems)-1;
    for ($i = 0; $i < $size; ++$i){
        if($availablegradeitems[$i]['id'] == $gradeitemidtarget){
            return $i;
        }
    }
    return -1;
...
function select_grade_item($grade_items, $grade_item_id){
...
function generate_id_number($grade_item_id){
    //@TODO Falta definir nomenclatura per als idnumbers
    return "Exam." . ($grade_item_id - 1);
}

function afegir_idnumber($item, $result){
    if (isset($item)) $item->add_idnumber($result);
}

function obtain_idnumber($grade_item_id, $grade_items){
    $item = select_grade_item($grade_items, $grade_item_id);
    if (!empty($item->idnumber)) return $item->idnumber;
    $result = generate_id_number($grade_item_id);
    afegir_idnumber($item, $result);
    return $result;
}
...
function generate_formula_moodle($items){
    $formula = "=" . check_operation($items[0]['operation']);
