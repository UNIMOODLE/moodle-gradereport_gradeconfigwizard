...
class formulamanager {
    ...
    public static function preprocess_formula_xml($formulaxml, $allgradeitem) {
        ...
        for ($i = 0; $i < sizeof($elements['ITEM']); $i++) {
            ...
            foreach ($items as $item) {
                ...
                $itemgrade['idnumber'] = \gradereport_gradeconfigwizard\formulamanager::obtain_idnumber($item['GRADEITEMID'][0]['#'],$allgradeitem);
    ...
    private static function select_grade_item($gradeitems, $gradeitemid) {
        foreach ($gradeitems as $item) {
            if ($item->id == $gradeitemid) return $item;
        }
        return null;
    }
