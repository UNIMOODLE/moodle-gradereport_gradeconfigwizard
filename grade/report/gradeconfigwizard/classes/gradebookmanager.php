...
class gradebookmanager{
    ...
    // Gradebook Creations
    public static function process($courseid, $relativepaths, $randomnames_dictionary, $newgradeitems, $gradeitemparent){
        // In case there are new categories to create
        if($relativepaths != null) $randomnames_dictionary = self::create_categories($randomnames_dictionary, $relativepaths, $courseid);
        // In case there are new grade items to create
        if($newgradeitems != null) self::create_grade_items($gradeitemparent, $courseid, $randomnames_dictionary, $newgradeitems);
    ...
    private static function create_grade_items($gradeitemparent, $courseid, $randomnames_dictionary, $newgradeitems){
        ...
        foreach($gradeitemparent as $key => $value){
            ...
            if(count($parts) == 2){
                ...
                // Otherwise
                self::create_grade_item($randomnames_dictionary[$parts[0]]['categoryid'], $newgradeitems[$key], $courseid);
            }
        }
    }
    ...
    private static function create_categories($randomnames_dictionary, $relativepaths, $courseid){
        // Init dictionary names
        $new_dictionary_names = [];
        foreach ($randomnames_dictionary as $key => $value) {
            $new_dictionary_names[$key]['categoryid'] = -1;
            $new_dictionary_names[$key]['name'] = $value;
        }
        $randomnames_dictionary = $new_dictionary_names;
        ...
        for($i = 1; $i <= self::get_max_depth($relativepaths); $i++){
            foreach ($relativepaths as $items) {
                ...
                if(count($parts) == 2){
                    $new_category_id = self::create_subcategory(
                        $parts[0],
                        $randomnames_dictionary[$parts[count($parts)-1]]['name'],
                        $courseid);

                    $randomnames_dictionary[$parts[count($parts)-1]]['categoryid'] = $new_category_id;
                }else{
                    // Case when the category parent is a subcategory
                    $new_category_id = self::create_subcategory(
                        $randomnames_dictionary[$parts[count($parts)-2]]['categoryid'],
                        $randomnames_dictionary[$parts[count($parts)-1]]['name'],
                        $courseid);
                    $randomnames_dictionary[$parts[count($parts)-1]]['categoryid'] = $new_category_id;
                }
    ...
    public static function get_grade_items($courseid)
    {
        ...
        // Erase Category and total of the main course
        //array_shift($availablegradeitems);
