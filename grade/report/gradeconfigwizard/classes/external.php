...
class gradereport_gradeconfigwizard_external extends external_api {
    ...
    public static function get_course_grades($userid = 0) {
        ...
        $gpr = new grade_plugin_return(array('type' => 'report', 'plugin' => 'gradeconfigwizard', 'courseid' => $course->id,
                                        'userid' => $userid));
        $report = new grade_report_gradeconfigwizard($userid, $gpr, $context);
    ...
    public static function view_grade_report($courseid, $userid = 0) {
        ...
        $access = grade_report_gradeconfigwizard::check_access($systemcontext, $context, $personalcontext, $course, $userid);

        if (!$access) {
            throw new moodle_exception('nopermissiontoviewgrades', 'error');
        }

        grade_report_gradeconfigwizard::viewed($context, $course->id, $userid);
