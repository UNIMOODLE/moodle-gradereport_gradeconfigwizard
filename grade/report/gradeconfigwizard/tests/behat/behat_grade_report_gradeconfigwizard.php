...
class behat_grade_report_gradeconfigwizard extends behat_base {
    ...
    /**
     * Choose the student i the select
     *
    ...
    public function i_choose_student($student){
        if ($this->getSession()->getPage()->find('css', 'select:contains("Choose...")')){
            ...
            $user = "user";
            $this->execute('behat_grade::i_click_on_in_search_widget', array($student, $user));
        }
    }
    ...
    public function i_check_formula_is_correct($course, TableNode $data){
        ...
        $formula = "";
        $gtotcat = grade_item::fetch_all(array('itemtype'=>'category', 'courseid'=>$course->id));
        foreach ($datahash as $key => $value) {
            ...
            foreach ($elements as $gitem){
                try{
                    $g = grade_item::fetch(array('itemname'=>$gitem, 'courseid'=>$course->id));
                    if ($g) $elements_id[] = $g->id;
                    else{
                        foreach ($gtotcat as $gcat){
                            if (str_contains($gcat->idnumber, $gitem)) $elements_id[] = $gcat->id;
                        }
                    }
                } catch (Exception $e){
                    echo $e->getMessage();
        ...
        if($value_textarea != $formula)
            throw new ExpectationException("Formula is not correct, expected: " . $value_textarea . " but found: " . $formula, $this->getSession());
    }
    ...
    public function i_add_categories_to_the_with_the_following_values($category, TableNode $data) {
        ...
        $sequentialArray = [];
        foreach ($datahash as $key => $value) $sequentialArray[] = [$key, $value];
        ...
        for ($i = 0; $i < count($datahash); $i++) {
            ...
            $bol = false;
            foreach ($rows as $row) {
                if (strpos($row->getText(), $category) !== false) $bol = true;
                if($bol) $rowsunder[] = $row;
            }
            // Find the add pathway button of the category in the rowsunder
            foreach ($rowsunder as $row) {
                if ($button = $row->find('css', 'button:contains("Add category")')){ $button->click(); break;}
            }
        }
        ...
        foreach ($inputs as $input) {
            if ($input->isVisible() && ($input->getValue() == "" || $input->getValue() == 1)) {
                ...
                if (isset($sequentialArray[$i / 2][0]) && isset($sequentialArray[$i / 2][1])) {
                    if ($i % 2 == 0) {
                        $input->setValue($sequentialArray[$i / 2][0]);
                    } else {
                        $input->setValue($sequentialArray[$i / 2][1]);
                    }
                }
                $i++;
            }
    ...
    public function i_add_the_following_items_to_the_with_the_following_values($category, TableNode $data) {
        ...
        foreach ($rows as $row) {
            ...
            if($var) {
                ...
                if ($button = $row->find('css', 'button:contains("Add item")')) {
                    $button->click();
                    break;
                }
            }
        ...
        foreach ($inputs as $input) {
            ...
            if(!$bol){
                foreach ($tds as $td) {
                    if (strpos($td->getText(), array_keys($datahash)[0]) !== false) $bol = true;
                }
            }
            if($bol){
                foreach ($tds as $td) {
    ...
    public function i_add_the_following_items_to_the_category($category, TableNode $data) {
        ...
        foreach ($trs as $tr) {
            if(strpos($tr->getText(), $category) !== false) $var = true;
    ...
    public function i_should_see_the_following_formulas(TableNode $data) {
        ...
        foreach ($datahash as $key => $value) {
            ...
            if($formula != $value) throw new ExpectationException("The formula is not correct, expected: " . $value . " but found: " . $formula, $this->getSession());
     ...
     * are correct.
     *
     * @Then /^I check formula in the item "([^""]*)" from "([^""]*)"$/
     */
    public function i_check_formula_in_item_from_course($item, $course){
        GLOBAL $DB;
        $row = $this->getSession()->getPage()->find('css', 'tr:contains("'.$item.'")');
        $row->find('css', 'a:contains("Edit")')->click();
        $row->find('css', 'a:contains("Edit calculation")')->click();
        ...
        $course = $DB->get_record('course', array('fullname'=>$course), '*', MUST_EXIST);
        try{
            $grade_category_total = grade_item::fetch_all(array('itemtype'=>'category', 'courseid'=>$course->id));
        } catch (Exception $e){
            echo $e->getMessage();
        }
        $result = '';
        foreach ($grade_category_total as $total) {
            if($total->calculation == $formula) $result = $total->calculation;
        }

        if($result == '') throw new ExpectationException("Incorrect Formula generation", $this->getSession());
        $this->getSession()->back();
    }
